...
const ReviewCard = ({ username, date, rating, title, comment, isOwner, onEdit, onDelete }: IReviewCardProps) => {
  ...
  return (
    <div className="w-full border-b border-gray-200 pb-6 last:border-none">
        ...
        {rating !== undefined && rating > 0 ? (
          <div className="flex items-center gap-2" aria-label={`Note de ${rating} sur 5 étoiles`}>
            {Array.from({ length: 5 }).map((_, i) => (
              <svg key={i} xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill={i < fullStars ? "#FBBF24" : "#E5E7EB"}>
      ...
      {title && <h4 className="text-lg font-semibold text-black mb-2 dark:text-yellow-500">{title}</h4>}
      {comment && <p className="text-lg text-gray-600 leading-8 dark:text-placeholder">{comment}</p>}

      {isOwner && (
        <div className="flex gap-4 mt-4">
          <button onClick={onEdit} className="px-4 py-2 me-2 mb-2 rounded-lg bg-blue-300 font-medium hover:bg-blue-200 transition dark:text-gray-800">
            <i className="fa-solid fa-pencil pr-2"></i> Modifier
          </button>

          <button onClick={onDelete} className="px-4 py-2 me-2 mb-2 rounded-lg bg-red-300 font-medium hover:bg-red-200 transition dark:text-gray-800">
